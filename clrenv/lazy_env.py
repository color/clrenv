    ...
    if not mode in _env:
        ...
        if len(set(mode) - allenvs) != 0:
            raise EnvironmentError(
                "Modes %s not defined anywhere" % (set(mode) - allenvs)
            )
        ...
        for k, v in overrides:
            for pytype in (safe_load, eval, int, float, str):
                ...
                print(
                    "Failed to convert %s into anything sensible!" % v, file=sys.stderr
                )
                sys.exit(1)
        ...
        if isinstance(value, dict):
            ...
            if value.startswith("^keyfile "):
                ...
                if os.environ.get(OFFLINE_FLAG):
                    logger.warning(
                        f"[{socket.gethostname()}] Offline, using placeholder value for {parameter_name}."
                    )
                    value = OFFLINE_VALUE
                else:
                    try:
